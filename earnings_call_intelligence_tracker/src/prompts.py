...
TEMPORAL_COMPARISON_PROMPT = """Temporal comparison task:
- Use historical transcript context only when provided.
- Fill temporal_comparison with:
  tone_change: whether current management tone is more cautious, more positive, similar, or mixed versus history;
  recurring_topics: current-call topics that were already recurring historically;
  new_or_escalating_topics: topics that appear new, sharper, more quantified, or more urgent than history;
  analyst_pressure_change: how analyst pressure changed versus historical Q&A patterns;
  historical_context_summary: one concise sentence explaining what history changes in the interpretation;
  confidence: analytical confidence in the temporal comparison.
- If no historical transcript context is provided, leave temporal_comparison fields null or empty.
- Do not cite historical quotes as evidence for the current call.
- Do not treat a historically recurring topic as a major surprise unless the current call materially changes guidance, numbers, language, or urgency."""
...
- Write structured summary fields as complete standalone sentences.
- Do not use ellipses ("...") as placeholders or to indicate omitted content.
...
- temporal_comparison recurring_topics must contain no more than 6 items.
- temporal_comparison new_or_escalating_topics must contain no more than 5 items.
...
def build_analysis_prompt(
    *,
    ...
    history_block = _context_block(
        title="Historical transcript context",
...
- Use historical transcript context only to identify recurring vs new topics, tone shifts, recurring analyst pressure, and guidance/theme continuity.
- Do not use historical context as literal evidence for the current call.
- Do not copy historical quotes into current-call evidence.
- If a current-call topic was already recurring historically, avoid treating it as a major surprise unless the current call materially changes language, numbers, guidance, or urgency.
...
def build_self_critique_prompt(
    *,
    ...
    quality_context = {
        ...
        "invalid_quotes": evidence_report.get("invalid_quotes", []),
        "evidence_warnings": evidence_report.get("warnings", []),
        "consistency_summary": consistency_report.get("summary", {}),
        "consistency_issues": consistency_report.get("issues", []),
    }
    quality_json = json.dumps(quality_context, ensure_ascii=False, indent=2)

    return f"""Self-critique task:
...
Do not use outside knowledge.
Do not invent replacement quotes.
Do not add fields.

Correct only issues that are clear from the evidence and consistency reports:
- remove or weaken analytical claims supported only by invalid evidence;
- downgrade confidence when evidence is weak, approximate, invalid, or internally inconsistent;
- remove exaggerated red flags if the quote validation or consistency report does not support them;
- reduce speculative surprise-score claims when the context or consistency report says support is weak;
- fix internal inconsistencies across surprise score, consensus context, limitations, and evidence validation.
...
If the deterministic reports show no material problem, return the same JSON with minimal or no changes.
Return one complete valid JSON object that follows the schema exactly.
Summary language preference inside JSON fields is: {language}
