    ...
    def send_message(
        ...
        *,
        ...
        try:
            ...
            raise AnthropicClientError(_format_anthropic_error(exc)) from exc
...
def _format_anthropic_error(exc: Exception) -> str:
    ...
    if error_type == "APIConnectionError":
        return (
            "Nao foi possivel conectar a API da Anthropic. "
            "Verifique conexao com a internet, proxy/VPN/firewall ou reinicie o Streamlit fora do sandbox. "
            f"Detalhe: {detail or error_type}"
        )

    if status_code:
        return (
            "Erro ao chamar a API da Anthropic. "
            f"Status HTTP {status_code} ({error_type}). "
            f"Detalhe: {detail or 'sem detalhe retornado'}"
        )

    return (
        "Erro ao chamar a API da Anthropic. "
        "Verifique chave, modelo, conexao e limites de uso. "
        f"Detalhe: {detail or error_type}"
    )
