...
def test_earnings_call_analysis_accepts_minimal_valid_json() -> None:
    ...
    assert analysis.temporal_comparison.recurring_topics == []
...
def test_earnings_call_analysis_accepts_temporal_comparison() -> None:
    payload = _minimal_valid_analysis()
    payload["temporal_comparison"] = {
        "tone_change": "Current tone is more cautious than recent calls.",
        "recurring_topics": ["asset quality", "capital"],
        "new_or_escalating_topics": ["credit card contamination"],
        "analyst_pressure_change": "Analysts pressed harder on provisions.",
        "historical_context_summary": "History frames asset quality as recurring but credit card contamination as newer.",
        "confidence": "medium",
    }

    analysis = EarningsCallAnalysis.model_validate(payload)

    assert analysis.temporal_comparison.tone_change is not None
    assert analysis.temporal_comparison.new_or_escalating_topics == ["credit card contamination"]


def test_earnings_call_analysis_rejects_too_many_temporal_topics() -> None:
    payload = _minimal_valid_analysis()
    payload["temporal_comparison"] = {
        "recurring_topics": ["a", "b", "c", "d", "e", "f", "g"],
